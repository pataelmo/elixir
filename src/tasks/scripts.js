...
Elixir.extend('scripts', (scripts, output, baseDir) => {
    const paths = prepGulpPaths(scripts, baseDir, output);

    new Elixir.Task('scripts', () => {
...
Elixir.extend('scriptsIn', (baseDir, output) => {
    const paths = prepGulpPaths('**/*.js', baseDir, output);

    new Elixir.Task('scriptsIn', () => {
        return gulpTask.call(this, paths);
    })
    .watch(paths.src.path)
    .ignore(paths.output.path);
...
Elixir.extend('babel', (scripts, output, baseDir, options) => {
    const paths = prepGulpPaths(scripts, baseDir, output);

    new Elixir.Task('babel', () => {

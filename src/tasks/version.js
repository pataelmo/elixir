...
const publicPath  = Elixir.config.publicPath;
...
Elixir.extend('version', (src, buildPath) => {
    const paths = prepGulpPaths(src, buildPath);

    loadPlugins();

    new Elixir.Task('version', () => {
        ...
        return (
            gulp.src(paths.src.path, { base: `./${publicPath}` })
                ...
                .on('end', () => {
                    ...
                    del(files.paths.filter(file => fs.lstatSync(file).isFile()), { force: true });
 ...
 * @return {GulpPaths}
 */
var prepGulpPaths = (src, buildPath) => {
 ...
 */
var emptyBuildPathFiles = (buildPath, manifest) => {
